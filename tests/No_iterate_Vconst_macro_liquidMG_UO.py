...
def NUCLEA_run():
    ...
    Tmin = 2800.0
    Tmax = 4400.0
    Trange = np.linspace(Tmin, Tmax, num=64, endpoint=False)
    results = pd.DataFrame(columns=['temperature', 'n_phase1', 'n_phase2', 'xU_phase1', 'xU_phase2', 'xU_interface', 'sigma','partial_molar_volumes_O','partial_molar_volumes_U','functions'])
        ...
        if (set(phasesAtEquilibriumMolarAmounts)==set(['LIQUID#1', 'LIQUID_AUTO#2'])):
            ...
            if (np.abs(sigma.Interfacial_Energy.values)>1E-6):
                ...
                results = results.append({'temperature' : T,
                                        ...
                                        'partial_molar_volumes_O' : functions[0](x0),
                                        'partial_molar_volumes_U' : functions[1](x0),
                                        'functions': functions,
                                        },
                        ignore_index = True)
            else:
    ...
    results.to_csv('no_iterate_ConstV_nuclea.csv')
    
    return results
    
def TAFID_run():
    results1 = NUCLEA_run()
    print ('functions=',results1['functions'],'\n')
    print('### test U-O coherent interface in the liquid miscibility gap ###\n')
    # tdb filepath
    # tdbFile=os.environ['TDBDATA_PRIVATE']+'/feouzr.tdb'
    #tdbFile=os.environ['TDBDATA_PRIVATE']+'/NUCLEA-17_1_mod.TDB'
    ...
    Tmin = 2800.0
    Tmax = 4400.0
    Trange = np.linspace(Tmin, Tmax, num=60, endpoint=False)
    #Tmin = 2800.0
    #Tmax = 2900.0
    ...
    for T in Trange:
        ...
        print('at T=', T, ' out of the miscibility gap')
    # print('phases at equilibrium:', TAFID_run.phasesAtEquilibriumMolarAmounts)
    # write csv result file
    results.to_csv('No_iterate_ConstV.csv')

def fit():
    results = pd.read_csv('No_iterate_ConstV.csv')
